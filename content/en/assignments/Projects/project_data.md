...
execute the workflow with `sbatch` within the `cl_sbatch_run` directory as follows: 
   ...
   ```
Note, some users might need to change in the `wf_run_script.R` "gen242" to "epyc" under the `partition` argument. After the submission to the cluster, one usually should check its status and progress with `squeue -u <username>` as well a 
