...
files to this subdirectory. Next, the parameters in both files need to be adjusted to match the type of workflow and
the required computing resouces. This includes the name of the `Rmd` file, and scheduler resource setting such as: 
`partition`, `Njobs`, `walltime`, `memory`, etc. After all relevant settings have been checked, one can execute the 
workfow from within the `cl_sbatch_run` directory with `sbatch` as follows: 
   ```sh 
   sbatch wf_run_script.sh
   ```
After the submission to the cluster, one usually should check its status and progress with `squeue -u <username>` as well 
by monitoring the content of the `slurm-<jobid>.out` file generated by the scheuler in the same directory. This file 
contains most of the `STDOUT` and `STDERROR` generated by a cluster job. Once everything is running, users can run the
workflow on the real data the same way. 
