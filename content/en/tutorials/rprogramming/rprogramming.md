...
A [shebang](https://en.wikipedia.org/wiki/Shebang_\(Unix\)) needs to be included at the beginning of an R script in order to execute it just with `./myscript.R` after making it executable (see [here](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/linux/linux/#permissions-and-ownership) for UNIX-like OSs). The shebang line is optional when executing R scripts from the R console with the `source` command or with `Rscript` from the command-line. The shebang of R scripts looks as follows:
