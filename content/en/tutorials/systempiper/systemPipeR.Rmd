...
This section introduces the usage of _`systemPipeR`_ on computer clusters.
...
the use of template files (_`*.tmpl`_) for defining the run parameters of
different schedulers. To run the following code, one needs to have both a
`conffile` (see _`.batchtools.conf.R`_ samples
...
The above example would submit via `runWF(sal)` the *hisat2_mapping* step
