...
`systemPipeR` workflows from the command-line to a HPC cluster by including the
relevant workflow run instructions in an R script and then submitting it via a
submission script of a workload manager system to a computer cluster. The
following gives an example for the Slurm workload manager using the RNA-Seq
workflow as an example. To understand the details, it is important to inspect
the content of the two files (here .R and .sh). Additional details about
resource specification under Slurm are given
