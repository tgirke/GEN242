...
For learning purposes one can use the much smaller toy data and ChIP-Seq `Rmd`
...
In a real analysis report users want to delete the above instructions and then
extend this introduction section to provide all background information relevant
for their ChIP-Seq project.
...
This tutorial will demonstrate how to build the workflow in an interactive mode by appending each step
with the `appendStep` method. The `SYSargsList` object contains instructions for
