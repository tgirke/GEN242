...
<p align="right">
  ...
  <a href="https://girke.bioinformatics.ucr.edu/GEN242/slides/aitutor/aitutor.html" target="_blank" rel="noopener noreferrer" style="text-decoration:none">
...
This tutorial introduces three AI coding tools and shows how to use them
...
This section covers manual installation of all three tools. Going through
...
- **Node.js** — required prerequisite for Copilot and Antigravity CLI (not needed for Claude Code)
...
section covers all three AI tools including Node.js.
:::
...
```bash
module spider node     # search all available modules
module load nodejs     # load if found
node --version
```
...
RStudio Desktop. For RStudio Server and OnDemand, Copilot is disabled by
default and must be enabled by a system administrator — check with your
HPCC admins before assuming it is available there.

No plugin installation needed. Option names may vary slightly depending on
your RStudio version:
...
**Chat interface in RStudio:** RStudio 2026.04 and later includes **Posit
Assistant** — a conversational chat panel similar to CopilotChat.nvim. It
appears as a panel in the IDE and understands your current R session context
(variable names, data frame columns, console history). Enable it via the
**Posit Assistant** button in the toolbar. Note that on RStudio Server and
Posit Workbench, Posit Assistant is also disabled by default and requires
administrator enablement — and is currently marked experimental on those
platforms. On RStudio Desktop it works without any extra configuration.
...
Here is a typical example of looking up a ggplot2 scatter plot:
...
data — and you can ask it to explain every line. The shift is from copying
to conversing.
...
Throughout this tutorial you will use the same task with each tool:

> *"Using the `diamonds` dataset from ggplot2, create a visualization that
> explores the relationship between carat, price, and cut. Then explain
> what the code does and suggest one improvement."*

The `diamonds` dataset has 53,940 rows and variables including `carat` (weight),
`price` (in USD), `cut` (quality: Fair to Ideal), `color`, and `clarity`.
It is built into ggplot2 — no loading required beyond `library(ggplot2)`.

This task is open-ended by design. There is no single correct plot.
Different tools will make different choices — comparing them is part of the
learning.
...
press `Space-ce` to open CopilotChat with an explain request. Read the
explanation — does it match what the code actually does?
...
> **Setup:** see the [Installation](#installation) section above. Note that
> on RStudio Server / OnDemand, Copilot must be enabled by a sysadmin first.

#### Inline completion usage

Once enabled, suggestions appear automatically as grey inline text while
you type. Accept with `Tab`, dismiss with `Escape`. RStudio also adds a
Copilot panel in the top-right pane where you can browse multiple suggestions.

**GitHub Copilot Free tier:** if you are using the free Copilot tier (not
GitHub Education), RStudio will display a message when the monthly completion
limit is reached.

#### Posit Assistant (chat)

RStudio 2026.04 and later includes Posit Assistant — a conversational chat
panel that understands your current R session context. Click the **Posit
Assistant** button in the RStudio toolbar to open it. You can ask questions,
explain selected code, and request fixes in plain English — similar to
CopilotChat.nvim but inside RStudio.

Note: on RStudio Server and Posit Workbench (OnDemand), Posit Assistant is
disabled by default and requires administrator enablement.
...
Every Claude Code project should have a `CLAUDE.md` at the repo root.
...
run `claude` there. Claude edits files on disk; RStudio detects the changes
...
For tighter integration, the `ClaudeR` R package connects RStudio directly
to Claude Code via MCP, allowing Claude to execute R code in your active
session and see plot output in real time. Install with:

```r
# install.packages("remotes")
remotes::install_github("anthropics/ClaudeR")
```
...
## Comparison: Three Tools Side by Side

| Feature | Copilot (inline) | Copilot + CopilotChat | Claude Code | Antigravity CLI |
|---------|-----------------|----------------------|-------------|-----------------|
| Interaction style | Inline completion as you type | Inline + conversational chat | Conversational terminal | Conversational terminal |
| Reads whole project | No | No | Yes | Yes |
| Runs code | No | No | Yes | Yes |
| Explains reasoning | Limited (via comments) | Yes, in chat window | Yes, on request | Yes, on request |
| Neovim/tmux support | Plugin | Two plugins | Native terminal | Native terminal |
| RStudio support | Native (built-in) | Native (built-in) | Terminal tab or ClaudeR | Terminal tab |
| Cost | Free (GitHub Education) | Free (GitHub Education) | $20/month Pro | Free tier (rate-limited) |
| Best for | Syntax, idioms, fast completion | Completion + explanation without leaving Neovim | Complex tasks, multi-file, research | Free alternative to Claude Code |
