...
- **Claude Code** — a conversational terminal agent that reads and edits your
  project files directly (requires paid account)
...
- **OpenAI Codex** — OpenAI's terminal coding agent
  ...
  (free tier for ChatGPT; Codex via OpenAI API paid account)
...
::: {.callout-note}
## [New Posit Assistant in RStudio](https://posit.co/products/ai) (applies to situation in June 2026)

If you use RStudio, you may be wondering about **Posit Assistant** — the recently
released AI coding assistant built directly into RStudio. It is not covered as a separate
tool in this tutorial, but here is how it fits relative to the tools above.

**Posit Assistant** is well-suited for interactive work in RStudio because it 
has direct access to your live R session. It can see your loaded variables, console
output, and environment, making it a natural fit for exploratory work. It is 
currently fully functional only in RStudio Desktop, but not RStudio Server/OnDemand.

**Claude Code (and other AI Agents)** is a better choice for script and software writing within RStudio (introduced below). This applies to work on desktops, remote servers, 
HPC clusters and cloud systems. It operates as a terminal-based agent, reads and edits 
files directly, and handles larger codebases well. 

**A note on subscriptions:** both tools require their own paid subscription. Posit 
Assistant uses Posit AI as its managed backend,
while Claude Code requires a separate Anthropic subscription. Importantly,
even though Claude models power the Posit AI service, a Posit AI subscription
does not cover Claude Code. They are entirely independent services. 
:::
