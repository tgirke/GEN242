...
This tutorial introduces four AI-assisted coding tools for scientific data
analysis. The focus is on coding-based AI environments, where assistance is
integrated directly into tools such as Neovim, RStudio, Jupyter, VS Code, and
terminal-based workflows.

This differs from chat-based AI tools, which are useful for discussion,
explanation, and planning but are separated from the executable analysis
workflow. Coding-based environments allow scientists to generate, inspect,
edit, run, debug, and document code in context.

The goal is not only to produce working code faster, but to use AI assistance
to support efficient, reproducible, and well-reasoned data analysis.

### Tools Covered

This tutorial compares four AI-assisted coding tools that support scientific data analysis in coding-based environments.

- __GitHub Copilot:__ an inline coding assistant that suggests code completions while you type. It is well suited for interactive coding in editors such as RStudio, VS Code, and Neovim.
- __Claude Code:__ an agentic coding assistant that runs in the terminal. It can read project files, edit code, run commands, and help debug or extend an existing analysis workflow.
- __Antigravity CLI:__ Google’s terminal-based coding agent and successor to Gemini CLI. It provides command-line AI assistance for coding, debugging, and project-level development tasks.
- __OpenAI Codex CLI:__ OpenAI’s terminal-based coding agent. It is included for comparison because Codex is one of the major AI coding systems currently used in coding-based workflows.

Each section includes setup notes for both __Neovim/tmux__ and __RStudio-based__
workflows. The same simple data-analysis exercise is used across tools so
participants can compare how each environment supports code generation,
editing, execution, debugging, and documentation.

By working through the sections, participants can evaluate which AI coding
environment best fits their own analysis style, technical comfort level, and
reproducibility needs.


::: {.callout-note}

## [Posit Assistant in RStudio](https://posit.co/products/ai) (applies to situation in June 2026)

RStudio now includes Posit Assistant, an IDE-integrated AI assistant. It is not
covered as a separate tool in this tutorial, but it is useful for interactive
RStudio work because it can use live session context, including loaded objects,
console output, and the current environment.

For this tutorial, the main distinction is that Posit Assistant is best suited
for session-aware exploratory work inside RStudio, whereas terminal-based AI
coding agents such as Claude Code, Antigravity CLI, and OpenAI Codex CLI are
better suited for file-based coding, larger scripts, project-level edits, and
workflows on desktops, remote servers, HPC clusters, or cloud systems.

Posit Assistant and Claude Code also require separate subscriptions. A Posit AI
subscription does not provide access to Claude Code, even if Claude models are
used behind the Posit AI service.
...
**nvim-R-Tmux users:** the `setup_ai_tools.sh` script, that is introduced in the 
[Linux tutorial](https://girke.bioinformatics.ucr.edu/GEN242/tutorials/linux/linux.html#quick-configuration-on-ucrs-hpcc) 
and hosted in the [nvim-R-Tmux](https://github.com/tgirke/nvim-R-Tmux) repository, installs
...
**Account requirement:** Claude Pro (paid account) or API access via
...
an OpenAI API paid subscription.
...
**Cost:** Claude Pro required (paid account). API access via
...
runs code, and iterates on errors. Requires OpenAI API access (paid account) plus
an API key.

**Cost:** ChatGPT free tier available. Codex requires OpenAI API access (paid account)
...
| Cost | Free (GitHub Education) | Free (GitHub Education) | paid account | Free tier (rate-limited) | Free tier / paid account |
...
Codex is a natural choice. The cost of the required paid account is currently 
the same price as Claude Code.
