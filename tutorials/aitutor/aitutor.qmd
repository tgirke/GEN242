...
Both Copilot and Antigravity CLI require Node.js 20 or higher. Claude Code has its
own native binary installer and does not require Node.js. First check the version
of node available on your system:
...
This installs Node.js entirely within your home directory, no root access needed:
...
On the HPCC, load an appropriate version of nodejs:
...
This opens a browser window. Paste the device code shown in Neovim,
authorize the app. 

**nvim-R-Tmux users:** if you are using the
[nvim-R-Tmux](https://github.com/tgirke/nvim-R-Tmux) setup, run the
`setup_ai_tools.sh` script from that repository instead — it adds this
block automatically and also installs Claude Code and Antigravity CLI.

**Account requirement:** free for students and educators via GitHub Education
(`education.github.com`). Sign up with your `.edu` email address.

#### RStudio

Copilot is natively integrated in RStudio 2023.09 and later. It works in RStudio 
Desktop and RStudio Server (onDemand). No plugin installation is needed. The option
names of the following instructions may slightly vary depending on the exact RStudio
version:

1. **Tools → Global Options → Assistant → Enable GitHub Copilot**
2. Click **Sign In** and follow the browser authentication prompt
3. Accept suggestions. At the end select `Apply` and `OK`.
...
2026 as the successor to Gemini CLI. It requires Node.js 20+ (installed above):
...
### Quick reference: what requires Node.js
...
**What Copilot does not do:** it cannot fix errors autonomously, cannot
read output, and does not have a conversation. It is always you driving,
with Copilot filling in.

### Copilot with Neovim/tmux

> **Setup:** see the [Installation](#installation) section above —
> Node.js, the `copilot.lua` plugin block, `:Lazy sync`, and
> `:Copilot auth` are all covered there.
...
Then add another comment to ask for an explanation:

```r
# Explain what the above plot shows and suggest one improvement
```

Copilot will generate a commented explanation inline. Read it carefully —
does the explanation match what the code actually does?

**Learning tip:** after accepting a suggestion, read every line before
running it. Ask yourself: *do I understand why this geom was chosen?
What would happen if I changed the aesthetic mapping?*
...
you type. Accept with `Tab`, dismiss with `Escape`.
...
| Feature | Copilot | Claude Code | Antigravity CLI |
|---------|---------|-------------|-----------------|
| Interaction style | Inline completion as you type | Conversational terminal | Conversational terminal |
| Reads whole project | No | Yes | Yes |
| Runs code | No | Yes | Yes |
| Explains reasoning | Limited (via comments) | Yes, on request | Yes, on request |
| Neovim/tmux support | Plugin | Native terminal | Native terminal |
| RStudio support | Native (built-in) | Terminal tab or ClaudeR | Terminal tab |
| Cost | Free (GitHub Education) | $20/month Pro | Free tier (rate-limited) |
| Best for | Syntax, idioms, fast completion | Complex tasks, multi-file, research | Free alternative to Claude Code |

### When to use each

**Copilot** is best when you know roughly what you want to write and need
help with syntax, patterns, and boilerplate. It is fast, unobtrusive, and
works without any context-switching.

**Claude Code** is best for tasks that require reasoning — debugging,
refactoring, multi-file changes, and learning why something works the way
it does. The conversational model means you can push back, ask follow-ups,
and steer the output.
...
Copilot running continuously in the background for completion, and Claude
Code or Antigravity CLI available for when you need to reason through
