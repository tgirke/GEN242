...
```sh
srun --x11 --partition=short --mem=8gb --cpus-per-task 4 --ntasks 1 --time 20:00:00 --pty bash -l
...
result.
To avoid slowdowns of the load time of this page, ony 200 rows of the source table are loaded
below.

```{r show_GO_table, eval=TRUE}
BatchResult <- read.delim("results/GOBatchAll.xls")[1:200,]
library(DT)
datatable(BatchResult, options=list(scrollX=TRUE, autoWidth=TRUE))
```
