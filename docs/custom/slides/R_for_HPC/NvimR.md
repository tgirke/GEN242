...
output: 
  ioslides_presentation:
    ...
    df_print: paged
    smaller: true
subtitle: "Terminal-based R Environment" 
...
Rscript -e "rmarkdown::render('NvimR.Rmd'); knitr::knit('NvimR.Rmd', tangle=TRUE)"
-->
...
URLs: [1. main page](https://github.com/jalvesaq/Nvim-R), [2. HPCC manual](http://hpcc.ucr.edu/manuals_linux-cluster_terminalIDE.html), [3. custom installs](https://github.com/tgirke/Nvim-R_Tmux) 
...
but is not necessary for this basic exercise. If Nvim-R is not installed in a user's
account yet, then configure it with the `Install_Nvim-R_Tmux` command as outlined [here](https://girke.bioinformatics.ucr.edu/GEN242/custom/slides/R_for_HPC/NvimR.html#13).
...
![](NvimR_files/figure-html/nvim-r-tmux-demo_run-1.png)<!-- -->
...
instructions](https://github.com/tgirke/Nvim-R_Tmux) to install Nvim-R-Tmux from scratch on your own system (_e.g._ laptop or computer).
...
user accounts of UCR’s HPCC, the above install script `Install_Nvim-R_Tmux` includes the 
...
- To use a specific R version in Nvim-R, one simply loads it prior to starting Nvim. Instructions
  for enabling additional R version toggle options are provided [here](https://github.com/tgirke/Nvim-R_Tmux#rversion_toggle).
...
- Background
- Neovim-based IDE for R
- <div class="white">__Conclusions__</div>
- References
