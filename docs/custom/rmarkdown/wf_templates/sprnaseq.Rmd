...
For the first introduction to RNA-Seq analysis in GEN242, we will use the __`sprnaseq.Rmd`__ 
...
appendStep(sal) <- LineWise(
    code = {
        ...
        png("./results/fastqReport.png", units = "in", height = 18, width = 4*length(fqlist))
        seeFastqPlot(fqlist)
        dev.off()
...
appendStep(sal) <- LineWise(
    code = {
        ...
        write.table(BatchResult, "results/GOBatch.xls", row.names=FALSE, quote=FALSE, sep="\t")
        m <- useMart("plants_mart", dataset="athaliana_eg_gene", host="https://plants.ensembl.org")
