...
## (2.1) How to run translateDNA function
mypep <- translateDNA(x=myDNA, frame=1)
		# The three possible reading frames (1, 2 or 3) can be selected under the frame option.

## (2.2) Return result in data frame
data.frame(DNA=names(mypep), PEP=as.vector(mypep))  

## (2.3) Return result as AAStringSet object
library(Biostrings)
AAStringSet(mypep)

## (2.4) Perform same operation on DNAStringSet object from Biostrings
